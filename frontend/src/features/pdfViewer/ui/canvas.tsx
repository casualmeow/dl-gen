  ...
  return (
    <div className="relative flex flex-col items-center overflow-hidden h-screen">
      ...
      <Dock position="bottom" size="md">
        ...
        />
        <DockItem icon={<ZoomIn />} label="Zoom In" onClick={() => smoothZoom(scale + 0.2)} />
        <DockItem
          icon={<Hand />}
          label="Hand Tool"
          className={activeTool === 'pan' ? 'bg-muted' : ''}
          onClick={() => setActiveTool((t) => (t === 'pan' ? 'none' : 'pan'))}
        />
        <DockItem
          icon={<Search />}
          label="Magnifier"
          className={activeTool === 'magnifier' ? 'bg-muted' : ''}
          onClick={() => setActiveTool((t) => (t === 'magnifier' ? 'none' : 'magnifier'))}
        />
      </Dock>
